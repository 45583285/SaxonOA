...
import sl.activiti.JpdlModel;
import sl.activiti.Node;
import sl.activiti.Transition;
...
public class JpdlModelDrawer {
		...
		for (Node node : nodes.values()) {
			for (Transition transition : node.getTransitions()) {
				...
				Point p = GeometryUtils
				    .getRectangleLineCrossPoint(node.getRectangle(), new Point(xPoints[1], yPoints[1]), grow);
				if (p != null) {
					xPoints[0] = p.x;
					yPoints[0] = p.y;
				}
	...
	private void drawNode(Map<String, Node> nodes, Graphics2D g2, Font font) throws IOException {
		for (Node node : nodes.values()) {
			...
			if (nodeInfos.get(node.getType()) != null) {
				System.out.println(getClass().getResourceAsStream("/icons/48/" + nodeInfos.get(node.getType())));//
